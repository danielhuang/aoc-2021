...
fn all_moves(
    ...
    for (&pos, &tile) in grid.iter() {
        ...
        let mut targets = match tile {
            Tile::Amphipod('A') => vec![Coordinate2D(3, 2), Coordinate2D(3, 3)],
            Tile::Amphipod('B') => vec![Coordinate2D(5, 2), Coordinate2D(5, 3)],
            Tile::Amphipod('C') => vec![Coordinate2D(7, 2), Coordinate2D(7, 3)],
            Tile::Amphipod('D') => vec![Coordinate2D(9, 2), Coordinate2D(9, 3)],
            _ => continue,
        };
        if targets.get(1).copied() == Some(pos) {
            continue;
        }
        // if pos.1 == 1 {
        //     // in hallway
        ...
        //     }
        // }
        if pos.1 == 1 && (grid[targets[1]] != Tile::Path && grid[targets[1]] != tile) {
            targets = vec![];
        }
...
fn search(
    ...
    a_seen: &HashMap<char, Coordinate2D>,
) -> usize {
    let template = parse(
        "#############
#...........#
